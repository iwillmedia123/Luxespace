...
import { Property, Community, Developer, Agent } from "@/types";
import { db } from "@/lib/db";
import { formatAED, cn, getPropertyTypeLabel, getStatusLabel, getCompletionLabel } from "@/lib/utils";
...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    async function loadPropertyData() {
      ...
      setLoading(true);
      try {
        const data = await db.getPropertyBySlug(slug);
        if (data) {
          setProperty(data);
          ...
          // Related properties
          const related = await db.getRelatedProperties(data);
  ...
  if (loading) {
    return (
      <div className="bg-luxury-charcoal min-h-screen text-white pt-32 pb-24 animate-pulse flex flex-col justify-center items-center">
        <div className="w-12 h-12 border-2 border-luxury-gold border-t-transparent rounded-full animate-spin" />
        <span className="text-xs text-luxury-gold font-medium mt-4 tracking-widest uppercase">Loading Residence Dossier...</span>
      </div>
    );
  }
  ...
  if (!property) {
    return (
      <div className="bg-luxury-charcoal min-h-screen text-white pt-32 pb-24 flex flex-col items-center justify-center space-y-6">
        <div className="text-center space-y-2">
          <h2 className="font-serif text-2xl text-white">Residence Not Found</h2>
          <p className="text-xs text-gray-400 font-light max-w-xs mx-auto">
            The property dossier you are seeking may be archived, sold, or offline.
          </p>
        </div>
